...
/* Initialize */
   ...
   // SNTP_init(SOCKET_SNTP, g_sntp_server_ip, TIMEZONE, g_sntp_buf);
    ...
    // /* Get time */
      ...
      char message_date[10];
      char message_time[10];

//printf("date is %s",message);
 ...
 SNTP_init(SOCKET_SNTP, g_sntp_server_ip, TIMEZONE, g_sntp_buf);
    ...
    do
    {
        ...
        if (retval == 1)
        {
            lcd_set_cursor(0, 0);
                         sleep_ms(100);
                        sprintf(message_time ,"%d:%d:%d", time.hh, time.mm, time.ss);
                        if(time.ss <60)
                        lcd_string(message_date);
                        sleep_us(100);
                        sleep_ms(100);
                        lcd_set_cursor(1, 0);
                         sleep_ms(100);
                        sprintf(message_date ,"%d/%d/%d",time.dd, time.mo, time.yy);
